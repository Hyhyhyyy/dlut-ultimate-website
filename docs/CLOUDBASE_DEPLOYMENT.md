...
1. 手机和电脑都能打开网站。
2. `/recruit.html` 不包含表单或个人信息输入项，并明确“先入群、参加一次训练、再登记加入社团”。
3. 招新标准二维码和主题海报均能打开 `/recruit.html`，QQ 群码可长按保存，微信群动态邀请说明清晰。
4. 提交成员资料后，公开名册不会立即出现该成员。
5. 打开 `/admin.html`，只有指定管理员可以登录。
6. 管理员通过后，刷新任意设备均可看到该成员。
7. 拒绝或删除后，数据库记录和头像文件均被清理。
8. 从 `/photos.html` 提交照片后，照片进入后台“日常照片”审核队列。
9. 照片通过后显示在主站图库，删除后数据库记录和照片文件均被清理。
10. 已公开成员从 `/debut.html` 提交首次正式比赛记录，后台“首次比赛”队列能够审核。
11. 审核通过后，该成员进入公开名册“黑蚁飞盘队”分区，并显示“XX年入队”。
12. 重复真实姓名、重复昵称，以及姓名与昵称相同的成员登记均会被拒绝。
